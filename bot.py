...
async def gacha_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if image_url and image_url != "https://i.imgur.com/example1.jpg":
        try:
            ...
            # 如果图片发送失败，只发送文字
            logger.warning(f"图片发送失败: {str(e)}, URL: {image_url}")
            await update.message.reply_text(message, reply_markup=reply_markup)
    else:
        # 没有图片或使用示例图片，只发送文字
        await update.message.reply_text(message, reply_markup=reply_markup)
...
async def tenpull(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 检查最稀有角色
    rarity_priority = {"3星": 3, "2星": 2, "1星": 1}
    best = max(results, key=lambda x: rarity_priority[x[0]])
    best_rarity, best_name = best
    image_url = gacha.get_character_image(best_rarity, best_name)
    ...
    if image_url:
        try:
            ...
            logger.warning(f"十连抽图片发送失败: {str(e)}, URL: {image_url}")
...
async def test_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """测试图片发送功能"""
    test_image_url = "https://baimg.bigjackson.top/img/alu.jpg"  # 使用真实的图片URL
    message = "🖼️ 测试图片发送功能"
    ...
    try:
        ...
        await update.message.reply_text("✅ 图片发送成功！")
    except Exception as e:
        error_message = f"❌ 图片发送失败：{str(e)}\n\n可能的原因：\n1. 网络连接问题\n2. 图片URL无效\n3. Telegram API限制\n\n当前只支持文字模式"
